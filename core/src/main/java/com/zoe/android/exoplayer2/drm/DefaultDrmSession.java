...
import com.zoe.android.exoplayer2.util.EventDispatcher;
...
/* package */ class DefaultDrmSession<T extends ExoMediaCrypto> implements DrmSession<T> {
  ...
  public final @Nullable List<DrmInitData.SchemeData> schemeDatas;
  ...
  private class PostRequestHandler extends Handler {
    ...
    public void handleMessage(Message msg) {
      ...
      try {
        switch (msg.what) {
          case MSG_PROVISION:
            response = callback.executeProvisionRequest(uuid, (ExoMediaDrm.ProvisionRequest) request);
            break;
          case MSG_KEYS:
            response = callback.executeKeyRequest(uuid, (ExoMediaDrm.KeyRequest) request);
            break;
          default:
