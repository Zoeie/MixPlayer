...
/* package */ final class PlaybackInfo {
   ...
   */
  public MediaSource.MediaPeriodId getDummyFirstMediaPeriodId(
    ...
    int firstPeriodIndex =
        timeline.getWindow(timeline.getFirstWindowIndex(shuffleModeEnabled), window)
            .firstPeriodIndex;
    return new MediaSource.MediaPeriodId(timeline.getUidOfPeriod(firstPeriodIndex));
  }

  /**
   * Copies playback info and resets playing and loading position.
   *
   * @param periodId New playing and loading {@link MediaSource.MediaPeriodId}.
  ...
  public PlaybackInfo resetToNewPosition(
          MediaSource.MediaPeriodId periodId, long startPositionUs, long contentPositionUs) {
    return new PlaybackInfo(
        timeline,
        manifest,
        periodId,
